...
class Site_Job_ExportPage extends Erfurt_Worker_Job_Abstract
{
    ...
    public function run($workload)
    {
        ...
        if (strpos($workload->resourceUri, $workload->urlBase) !== 0) {
            ...
            // create filename for dump based on url
            $nameparts = explode('/', $name);
            $filename = array_pop($nameparts);
            $dirname = $workload->targetPath . implode('/', $nameparts);
            
            $postponeable = isset($workload->postponeable) ? $workload->postponeable : false;
            
            if (!file_exists($dirname . '/' . $filename) || !$postponeable) {
                $datawiki = OntoWiki::getInstance();
                ...
                $cache = null; unset($cache);
                    ...
                    echo $workload->progress . ' ' . 'Found dump, postpone re-creation.' . PHP_EOL;
                    $this->logSuccess($workload->progress . ' ' . 'Found dump, postpone re-creation.');
                    $workload->postponeable = false;
                    $datawiki = OntoWiki::getInstance();
                    $datawiki->callJob(
                        'exportPage',
                        $workload
